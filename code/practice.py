...
def A(variables_a,variables):
    ...
    w,pi,k,Tx,Ty,Tz,s,x,y,z = symbols('w pi k Tx Ty Tz s x y z')
...
# Variables_a needs updating
def B(variables_a,variables):
    counter = 0
    other_counter = 0
    B = np.zeros([12,24],dtype=float)
    w,pi,k,Tx,Ty,Tz,s,x,y,z,Xb,Yb,Zb = symbols('w pi k Tx Ty Tz s x y z Xb Yb Zb')
...
# First First Design Matrix
w,pi,k,Tx,Ty,Tz,s = symbols('w,pi,k,Tx,Ty,Tz,s')
variables_a = [(w,omega),(pi,phi),(k,kappa),(Tx,T_x),(Ty,T_y),(Tz,T_z),(s,scale)]
a1 = A(variables_a,variables)
A1 = np.zeros([12,7],dtype=float)
A1[:] = a1[:]

# First Second Design Matrix
b1 = B(variables_a,variables)
